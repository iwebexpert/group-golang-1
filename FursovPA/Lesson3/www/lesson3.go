...
func (a * PageArticles) createArticle (w http.ResponseWriter, r *http.Request) {
	...
	tempArticle := template.Must(template.New("createArticle").Parse(string(data)))
	tempArticle.ExecuteTemplate(w, "createArticle", a)
}
...
articles := PageArticles{
	Title: "Статьи",
	Articles: Articles{
		{ArticleID: 1, TitleArticle: "Статья 1", TextArticle: "Текст к статье 1"},
		{ArticleID: 2, TitleArticle: "Статья 2", TextArticle: "Текст к статье 2"},
		{ArticleID: 3, TitleArticle: "Статья 3", TextArticle: "Текст к статье 3"},
	},
}
